<template>
   <div>
    <q-card class="" style="width:105%;margin-left:-2.5%;">
      <q-card-section> 
        <q-item-section>
          ...
          <q-item-label style="margin-top:2%;">
             <q-select v-if="getEditar"  v-model="selectExperiencia" :options="options" label="Selecionar Educação" />
      ...
      <q-card-section style="margin-top:4%;" v-show="this.cargo != '' && this.empresa != '' && this.inicio != '' && this.saida != '' && !getEditar ">
        <q-item-label>
          <q-btn
            size="6px"
            round
      ...
      <q-card-section style="margin-top:5%;">   
       ...
       <ModalAdiciona :adiciona="adiciona" :desc1="'empresa'" :desc2="'cargo'" :dt1="'Inicio'" :dt2="'Saida'" @adiciona="adiciona= $event" />        
  </div>
</template>
...
export default {
  ...
  data () {
   ...
   mounted() {
    ...
    console.log(this.getExpProfissi);
  },
  computed: {
    ...mapGetters("jfs/",{
      getEditar: "getEditar",
  ...
  watch: {
    selectExperiencia(val){
        const experiencia = this.getExpProfissi
        .filter(element => element.cargo === val)
